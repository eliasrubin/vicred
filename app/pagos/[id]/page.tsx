...
type Venta = { id: string; fecha: string; factura_numero: string | null; total: number };
    ...
    (async () => {
      ...
      if ((p as any).venta_id) {
        const { data: v, error: vErr } = await supabase
          .from("ventas_credito")
          .select("id,fecha,factura_numero,total")
      ...
      // Detalle aplicado (si existe tabla pagos_aplicaciones)
      const { data: a, error: aErr } = await supabase
        .from("pagos_aplicaciones")
        .select("monto, cuota_id, cuotas!inner(nro,vencimiento,importe)")
        .eq("pago_id", id);

      // Si no tenés pagos_aplicaciones o el join falla, no frenamos el comprobante:
      if (!aErr && a) {
      ...
      <div style={{ border: "1px solid #eaeaea", borderRadius: 14, padding: 18, marginTop: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, flexWrap: "wrap" }}>
          <div>
            ...
            <p style={{ margin: "6px 0", opacity: 0.8 }}>Créditos VI (PCGAME / VIMODA)</p>
          </div>
